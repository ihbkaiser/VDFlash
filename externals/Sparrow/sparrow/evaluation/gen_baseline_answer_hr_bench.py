...
from .hr_bench_prompt import build_prompt
...
def _decode_image(example):
    ...
    example["image"] = Image.open(BytesIO((b64decode(example["image"]))))
    return example
...
def load_data(args):
    ...
    data = load_dataset("DreamMr/HR-Bench", split=args.split)
    data = data.shuffle(seed=42).select(range(0, 100))
    features = data.features.copy()
    features["image"] = Image()
    data = data.map(_decode_image, features=features, writer_batch_size=100)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split", type=str, default="hrbench_4k")
