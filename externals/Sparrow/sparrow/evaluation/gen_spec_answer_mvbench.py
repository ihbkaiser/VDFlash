...
from .mvbench_prompt import build_prompt
...
def load_data(args):
    ...
    for k, v in DATA_MAP.items():
        with open(
            os.path.join(args.data_folder, "json", f"{k}.json"), "r", encoding="utf-8"
        ) as f:
            datum = json.load(f)
            for d in datum:
                d["video"] = os.path.abspath(
                    os.path.join(args.data_folder, "videos", v, d["video"])
                )
            data.extend(datum)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-folder", type=str, default="vid_benchmark/MVBench")
