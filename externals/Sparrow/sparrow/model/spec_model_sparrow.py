...
from .cnets_sparrow import Model
...
from .modeling_llava_onevision_tree import LlavaOnevisionForConditionalGeneration as KVLlavaOnevisionForConditionalGeneration
...
class SpecModel(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        print("spec_model_msd_msd_msd_jianzhi")
        ...
        elif Type == "LlavaOnevisionForConditionalGeneration":
            base_model = KVLlavaOnevisionForConditionalGeneration.from_pretrained(
                base_model_path, **kwargs
            )
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        # torch.cuda.synchronize()
        # end_time = time.time()
        # target_elapsed = end_time - start_time


        if output_real_hidden:
            return outputs.hidden_states, orig, hidden_states, outputs.hidden_states
        if output_orig:
            return outputs.hidden_states, orig, hidden_states
        else:
            return outputs.hidden_states, hidden_states
    ...
    def specgenerate(
        ...
        **kwargs,
        ...
        # print("Ignored kwargs:", list(kwargs.keys()))
        ...
        if (
            ...
            vision_aspect_ratio = (
                vision_aspect_ratio if vision_aspect_ratio is not None else self.base_model.config.vision_aspect_ratio
            )
            ...
            if pixel_values is not None:
                ...
                n_image_tokens = (input_ids == self.base_model.config.image_token_index).sum().item()
            ...
            if pixel_values_videos is not None:
                video_features = self.base_model.get_video_features(
                    pixel_values_videos,
                    vision_feature_layer=vision_feature_layer,
                    vision_feature_select_strategy=vision_feature_select_strategy,
                ) #[bs, n_frame*196,dim]
                image_newline = (
                    self.base_model.image_newline[None, None, :].repeat(video_features.shape[0], 1, 1).to(video_features.device)
                )
                ...
                n_video_tokens = (input_ids == self.base_model.config.video_token_index).sum().item()
                ...
                special_video_mask = (
                    (input_ids == self.base_model.config.video_token_index)
                    .unsqueeze(-1)
                    .expand_as(inputs_embeds)
                    .to(inputs_embeds.device)
                )
                ...
                mask = input_ids == self.base_model.config.video_token_index
