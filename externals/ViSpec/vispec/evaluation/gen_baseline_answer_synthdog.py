...
from .synthdog_prompt import build_prompt


def load_data(args) -> Dataset:
    data = []
    with open(
        os.path.join(args.data_folder, "jsonl/gt.jsonl"), "r", encoding="utf-8"
    ) as f:
        lines = f.readlines()
        for l in lines:
            d = json.loads(l.strip())
            d["image"] = Image.open(open(os.path.join(args.data_folder, d["id"]), "rb"))
            data.append(d)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-folder", type=str, default="data/eval_synthdog_en_text")
