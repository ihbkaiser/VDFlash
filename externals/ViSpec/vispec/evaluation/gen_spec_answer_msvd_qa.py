...
from .msvd_qa_prompt import build_prompt


def load_data(args):
    with open(
        os.path.join(args.data_folder, "annotation_test.json"), "r", encoding="utf-8"
    ) as f:
        data = json.load(f)
        for d in data:
            d["video_name"] = os.path.abspath(
                os.path.join(args.data_folder, "videos/data", f"{d['video_name']}.avi")
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-folder", type=str, default="vid_benchmark/MSVD-QA")
