...
if int(args.gpus_per_model) >= 1:
    args.gpus_per_model = int(args.gpus_per_model)
    gpus = [
        [j for j in range(i, i + args.gpus_per_model)]
        for i in range(0, num_p, args.gpus_per_model)
    ]
else:
    models_per_gpu = int(1 / args.gpus_per_model)
    assert models_per_gpu >= 1
    gpus = [[i // models_per_gpu] for i in range(num_p * models_per_gpu)]
num_p = len(gpus)


outdir = "{}/qwen_pretrain_gen_{}_{}_mufp16".format(args.outdir, s, e)
...
for i in range(num_p):
    ...
    # )
    command = "{} -m vispec.ge_data.ge_data_all_qwen_pretrain_gen --start={} --end={} --index={} --gpu_index {} --outdir {} --model {} --temperature {}".format(
        sys.executable,
        start,
        end,
        index,
