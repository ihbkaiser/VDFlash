...
from ..model.cnets_medusa import Model
    ...
    for batch_idx, data in enumerate(
        ...
        with accelerator.accumulate(model):
            ...
            with torch.no_grad():
                ...
                data["target"] = torch.cat(tgt, dim=0)

                target_head = head(data["target"])
                target_p = nn.Softmax(dim=2)(target_head)
                target_p = target_p.detach()
            loss_mask = data["loss_mask"][:, :, None].expand(
                data["target"].shape[0], data["loss_mask"].shape[1], 1
            )
        ...
        with torch.no_grad():
            ...
            out_head = out_head.reshape(-1, target_head.shape[-1])[
