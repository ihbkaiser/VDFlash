...
_TRANSFORMERS_SUPPORTED_PATTERN = re.compile(
    r"qwen3[\w.-]*|llama.*3\.1.*8b.*instruct", re.IGNORECASE
)
...
def _check_transformers_model(model_name: str) -> None:
    if not _TRANSFORMERS_SUPPORTED_PATTERN.search(model_name):
        raise ValueError(
            ...
            f"Only Qwen3/Qwen3.5 series and LLaMA-3.1-8B-Instruct are supported. "
...
def _run_transformers(args: argparse.Namespace) -> None:
    ...
    # Tesla T4 (SM75) has no native BF16 tensor-core path; FP16 is the
    # practical dtype for this benchmark on that GPU generation.
    ...
    if "qwen3.5" in args.model.lower() or "qwen3_5" in args.model.lower():
        # Qwen3.5 is a multimodal conditional-generation model even for
        # text-only prompts; AutoModelForCausalLM selects its text-only class
        # and does not match the published checkpoint key layout.
        from transformers import AutoModelForImageTextToText

        model_cls = AutoModelForImageTextToText
    target = model_cls.from_pretrained(
        args.model, attn_implementation=attn_impl, dtype=dtype,
    ).to(device).eval()

    draft_model = DFlashDraftModel.from_pretrained(
        args.draft_model, attn_implementation=attn_impl, dtype=dtype,
    ).to(device).eval()
