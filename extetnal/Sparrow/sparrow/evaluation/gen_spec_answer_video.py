...
def get_model_answers(
    ...
    if args.use_vispec:
        print("use_vispec")
        from ..model.spec_model_vispec import SpecModel
    ...
    elif args.use_sparrow_VATA:
        print("use_sparrow_VATA")
        from ..model.spec_model_sparrow_VATA import SpecModel
    ...
    for _ in range(3):
        ...
        print("INFO: Input length:", model_inputs['input_ids'].shape[1])
    ...
    all_image_tokens_num = 0 
    all_text_tokens = 0
    target_decode_time = 0 
    target_decode_num = 0 
    draft_deocde_time = 0 
    draft_deocde_num = 0 
    for d in tqdm(data):
        ...
        for i in range(num_choices):
            ...
            print("INFO: Input length:", model_inputs['input_ids'].shape[1])
            image_tokens_num = (model_inputs['input_ids'] == 151656).sum().item()
            all_image_tokens_num += image_tokens_num
            all_text_tokens += (model_inputs['input_ids'].shape[1]-image_tokens_num)
            torch.cuda.synchronize()
            start_time = time.time()
            # print("model_inputs",model_inputs)
            output_ids, new_token, idx, accp_len , decode_time ,draft_prefill = model.specgenerate(
                **model_inputs,
            ...
            # # target_decode_num=target_decode_num,
            # # draft_deocde_time = draft_deocde_time,
            # # draft_deocde_num =draft_deocde_num,
            #  )
            torch.cuda.synchronize()
            total_time = time.time() - start_time
            output_ids = output_ids[0][len(model_inputs["input_ids"][0]) :]
    ...
    print("all_image_tokens_num",all_image_tokens_num)
    print("all_text_tokens",all_text_tokens)
    all_image_tokens_num = 0 
    all_text_tokens = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_vispec", type=bool, default=False)
    parser.add_argument("--num-q", type=int, default=2)
    parser.add_argument("--use_sparrow_VATA", type=bool, default=False)
    parser.add_argument("--use_sparrow", type=bool, default=False)
