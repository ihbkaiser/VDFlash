...
class Model(nn.Module):
    ...
    def forward(
        ...
        if position_ids is not None:
            past_key_values_real_length = position_ids.min()
        else:
    ...
    def topK_genrate(
        ...
        if is_qwen_2_5_vl :
            if inputs_embeds != None and 151656 in input_ids:
                image_tokens_num = [(input_ids[0] == 151656).sum().item()]
                indice = torch.where(input_ids[0] == 151652)[0].item() + 1
                hidden_states = torch.cat((hidden_states[:,:indice], hidden_states[:,indice + image_tokens_num[0]:]), dim=1)
                input_ids = torch.cat((input_ids[:,:indice], input_ids[:,indice + image_tokens_num[0]:]), dim=1)
            else:
                image_tokens_num = [(input_ids[0] == 151656).sum().item()]
                indice = torch.where(input_ids[0] == 151652)[0].item() + 1
                input_ids = torch.cat((input_ids[:,:indice], input_ids[:,indice + image_tokens_num[0]:]), dim=1)
            ...
            if inputs_embeds != None and 151647 in input_ids:
                image_mask_1 = input_ids == 151647
                img_positions = torch.where(image_mask_1[0])[0]
                start_idx = img_positions[0].item()
                end_idx = img_positions[-1].item()
                next_idx = end_idx + 1
                hidden_states = torch.cat((hidden_states[:,:start_idx], hidden_states[:,next_idx:]), dim=1)
                input_ids = torch.cat((input_ids[:,:start_idx], input_ids[:,next_idx:]), dim=1)
        ...
        hidden_states = hidden_states.to(self.embed_tokens.weight.device)
        ...
        # end_time = time.time()
        # draft_elapsed = end_time - start_time
        # with open("draft_elapsed.txt", "a") as f:
        #     f.write(f"{draft_elapsed:.6f}\n")
