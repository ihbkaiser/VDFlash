...
# Copied from transformers.intergrations.sdpa_attention.py
def sdpa_attention_forward(
    ...
    **kwargs,
    ...
    # attn_weight = attn_weight.mean(dim=1)  # [batch_size,seq_len_q, seq_len_k]
    # attn_weight = attn_weight.mean(dim=1)  # [batch_size, seq_len_k]

    return attn_output, attn_weight # [batch_size,num_head, seq_len_q, seq_len_k]
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if self.config._attn_implementation != "eager":
            ...
            # print(" self.config.sliding_window", self.config.sliding_window)
            if self.config._attn_implementation == "sdpa" and kwargs.get("output_attentions", False):
                ...
                # )
                # logger.warning_once(
                #     "Using sdap attention!"
                # )
                attention_interface = sdpa_attention_forward #MODIFIED
            else:
                attention_interface = ALL_ATTENTION_FUNCTIONS[self.config._attn_implementation]

        # if query_states.shape[2] == 1:
        #     attention_interface = eager_attention_forward #MODIFIED
        # sliding_window = None
...
class Qwen2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # torch.cuda.synchronize()
        # time3 = time.time()
        # print("Attn time:",time2-time1)
        # print("MLP time:",time3-time2)
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for idx,decoder_layer in enumerate(self.layers[: self.config.num_hidden_layers]):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                # if idx == idx_layer :
                #     # 假设 input_ids 是 [batch_size, seq_len] 的张量
                #     # 1. 计算视频 token 数量和起始位置
                #     batch_size = input_ids.shape[0]
                #     print("idx_layer",idx_layer)
                #     print("input_ids",input_ids.shape)
                #     image_tokens_num = (input_ids[0] == 151647).sum().item()  # 视频 token 的 ID (151656)
                #     # start_marker_pos = torch.where(input_ids[0] == 151652)[0].item()  # 视频开始标记位置
                #     mask = (input_ids[0] == 151647).float()
                #     # argmax 返回第一个最大值的位置，True (1.0) > False (0.0)
                #     indice = mask.argmax().item()   # 视频 token 的实际起始位置
                #     # indice = start_marker_pos + 1  # 视频 token 的实际起始位置
                #     end_idx = indice + image_tokens_num

                #      # 2.1 裁剪 hidden_states
                #     hidden_states = torch.cat((
                #         hidden_states[:, :indice],
                #         hidden_states[:, end_idx:]
                #     ), dim=1)
        ...
        **kwargs,
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            if attention_mask is not None:
                ...
                causal_mask = causal_mask.to(attention_mask.device) #MODIFIED
