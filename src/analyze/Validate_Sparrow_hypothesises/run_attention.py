...
def _calibration_jobs(
    ...
    for sample in samples:
        ...
        if not points:
            # A sample that failed calibration is skipped rather than aborting
            # the whole run; its absence is visible in the report row counts.
            print(f"WARNING: no calibration rows for {sample.sample_id}; skipping", flush=True)
            continue
