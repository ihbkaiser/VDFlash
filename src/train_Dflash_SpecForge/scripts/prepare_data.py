...
def load_dataset_from_path(data_path: Path) -> Any:
    ...
    try:
        ...
        # Reuse the prompt reader's tolerant JSON-array parser for partially
        # copied ShareGPT exports.  Complete rows before a truncated final
        # object are still valid training examples.
        try:
            from datasets import Dataset
            from specforge.data.prompt_builder import _iter_records

            records = [record for _, record in _iter_records(str(data_path))]
        except Exception:
            raise load_error
        if not records:
            raise load_error
        warnings.warn(
            f"Could not load {str(data_path)!r} as a complete JSON dataset; "
