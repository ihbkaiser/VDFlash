...
def _prepare_raw_prompts(
    ...
    *,
    ...
    try:
        ...
        # Hugging Face's JSON builder requires a complete JSON document.  A
        # partially transferred ShareGPT array can still contain many valid
        # records before its final truncated object; recover those records with
        # the tolerant iterator rather than dropping the whole training set.
        try:
            records = [record for _, record in _iter_records(path)]
        except Exception:
            raise load_error
        if not records:
            raise load_error
        ...
        warnings.warn(
            f"Could not load {path!r} as a complete JSON dataset; "
            f"recovered {len(records)} complete records and ignored the "
    ...
    with open(path, encoding="utf-8") as stream:
        ...
        if first_character == "[":
            try:
                ...
                # Keep complete records from a truncated transfer.  This is
                # deliberately limited to the trailing portion: a malformed
                # record in the middle stops iteration and emits a warning, so
                # callers never receive fabricated conversation content.
                stream.seek(0)
                text = stream.read()
                ...
                while True:
                    ...
                    try:
                        ...
                        warnings.warn(
                            f"invalid/truncated trailing JSON in {path!r}; "
                            f"recovered {recovered} complete records",
                            RuntimeWarning,
                        )
                        break
                    if not isinstance(record, dict):
                        raise ValueError(
                            f"JSON record {recovered + 1} in {path!r} must be an object"
                        )
                    recovered += 1
                    yield recovered, record
                    position = end
                    while position < len(text) and text[position].isspace():
                        position += 1
                    ...
                    if text[position] == "]":
                        break
                    warnings.warn(
                        f"unexpected trailing JSON in {path!r}; "
                        f"recovered {recovered} complete records",
                        RuntimeWarning,
                    )
                    break
