...
from specforge.data.prompt_builder import _iter_records, prepare_prompt_tasks
...
class TestPreparePromptTasks(unittest.TestCase):
    def test_truncated_json_array_recovers_complete_records(self):
        # ShareGPT exports are commonly JSON arrays.  A partial copy may end
        # inside the next object; complete records before that boundary remain
        # usable and the damaged tail must be ignored with a warning.
        payload = (
            '[{"id":"a","input_ids":[1,2],"loss_mask":[0,1]},'
            ' {"id":"b","input_ids":[3,4],"loss_mask":[1,1]},'
            ' {"id":"truncated","input_ids":'
        )
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with self.assertWarnsRegex(RuntimeWarning, "recovered 2 complete records"):
                records = list(_iter_records(path))
            self.assertEqual([record["id"] for _, record in records], ["a", "b"])
