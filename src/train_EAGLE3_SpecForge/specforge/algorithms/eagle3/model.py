...
class OnlineEagle3Model(Eagle3Model):
    ...
    def _prepare_position_ids(
        ...
        *,
        ...
        rope_scaling = getattr(self.draft_model.config, "rope_scaling", None) or {}
        if isinstance(rope_scaling, dict):
            rope_type = rope_scaling.get("rope_type", rope_scaling.get("type"))
        else:
            rope_type = getattr(rope_scaling, "rope_type", None)
        uses_mrope = rope_type == "mrope"
        ...
        if position_ids is None:
            ...
            if uses_mrope:
                # Text-only Qwen2.5-VL has identical temporal, height, and
                # width positions. Preserve the model's required [3, B, L]
                # M-RoPE contract without requiring a multimodal processor.
                return positions.view(1, 1, seq_length).expand(
                    3, batch_size, seq_length
                )
            return positions.unsqueeze(0).view(-1, seq_length)
        ...
        if uses_mrope and position_ids.ndim == 2:
