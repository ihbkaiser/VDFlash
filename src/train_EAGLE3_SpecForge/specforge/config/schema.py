...
class ModelConfig(StrictConfigModel):
    ...
    #: or a Hugging Face repository. EAGLE3 can derive it from the target when
    #: omitted.
    ...
    #: Optional EAGLE3 architecture override.
    draft_num_hidden_layers: Optional[int] = Field(default=None, gt=0)
    #: Retained for config compatibility; EAGLE3 does not use a block size.
...
class DataConfig(StrictConfigModel):
    ...
    #: Ordered background feature-loader workers.
...
class TrainingConfig(StrictConfigModel):
    strategy: Literal["eagle3"] = "eagle3"
