...
class TargetHead(nn.Module):
    ...
    def load_weights(
        ...
        if resolved_lm_head_key not in weight_map:
            tied_embedding_keys = (
            ...
            if not getattr(self.config, "tie_word_embeddings", False):
                raise KeyError(
                    f"{lm_head_key!r} is not present in {index_json_path}; "
                    "the target config does not declare tied word embeddings"
                )
            resolved_lm_head_key = next(
                (
                    key
                    for key in tied_embedding_keys
                    if key in weight_map
                ),
                None,
            )
            if resolved_lm_head_key is None:
                raise KeyError(
                    f"{lm_head_key!r} is not present in {index_json_path} and no "
                    "tied embedding weight was found"
                )
        ckpt_file = weight_map[resolved_lm_head_key]

        if ckpt_file.endswith(".safetensors"):
            with safe_open(
                os.path.join(self.model_path, ckpt_file), framework="pt"
            ) as f:
                lm_head = f.get_tensor(resolved_lm_head_key)
        else:
            state_dict = torch.load(os.path.join(self.model_path, ckpt_file))
            lm_head = state_dict[resolved_lm_head_key]
