...
class FSDPTrainingBackend(TrainingBackend):
    ...
    def _frozen_target_modules(model: nn.Module) -> tuple[nn.Module, ...]:
        ...
        EAGLE3 carries the frozen target embedding and LM head only for
        inference inside the loss. Sharding those tables makes FSDP all-gather
        them before every optimizer window without saving optimizer memory.
        """
        modules = []
    ...
    def prepare_model(
        ...
        *,
        ...
        if not wrap:
            ...
            # EAGLE3 models do not advertise block classes and retain a single
            # root FSDP unit.
            block_names = set(
