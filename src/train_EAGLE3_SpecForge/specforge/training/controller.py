...
``TrainerCore`` runs exactly one branch-free EAGLE3 step (strategy forward/loss,
backend backward/step) plus the grad-accumulation boundary.
``TrainerController`` owns the lifecycle: fit / evaluate / save_checkpoint.
"""
...
def _dp_mean_scalars(
    ...
    Average scalar metrics across data-parallel ranks before logging so each
    recorded value represents the full effective batch.
...
class TrainerCore:
    ...
    def _result(
        ...
        *,
        ...
        # Move CPU scalar diagnostics onto the loss device before the DP
        # reduction so NCCL-backed runs do not all-reduce a CPU tensor.
...
class TrainerController:
    ...
    :class:`Checkpoint`. Evaluation is configured once at construction time,
    so the public training lifecycle remains one no-argument :meth:`Trainer.fit`
    call.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Keep an optional schedule horizon distinct from max_steps. It falls
        # back to max_steps for a bounded run.
    ...
    def evaluate(self, data: Optional[Iterable[TrainBatch]]) -> Dict[str, Any]:
        ...
        module.eval()
        # Use the train path's live context for consistent evaluation semantics.
